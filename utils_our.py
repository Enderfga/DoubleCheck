        ...
        with open(path, 'r', encoding='UTF-8') as f:
            for line in tqdm(f):
                ...
                for sen in sentences:
                    ...
                    document.append((words_line, seq_len))
                sen_num = len(document)
                if 8:
                    if sen_num < 8:
                        for i in range((8 - len(document))):
                            document.append(([PAD] * pad_size, 0))
                    else:
                        document = document[:8]
                        sen_num = 8
                contents.append((document, int(label), sen_num))
        # print(contents)
        return contents  # [([...], 0), ([...], 1), ...]
...
class DatasetIterater(object):
    ...
    def _to_tensor(self, datas):
        x = []
        for _ in datas:
            for document, seq_len in _[0]:
                x.append(document)
        x = torch.LongTensor(x).to(self.device)
        y = torch.LongTensor([_[1] for _ in datas]).to(self.device)

        # pad前的长度(超过pad_size的设为pad_size)
        sen_len = torch.LongTensor([_[2] for _ in datas]).to(self.device)
        return (x, sen_len), y
...
def segment_to_para_sent(text, para_max_len):
    # 分段
    paras = []
    text = text.strip()
